...
import ltd.dnacreative.calendar.config.KafkaProperties;
...
class CareCloudCalendarKafkaResourceIT {
    ...
    void setup() {
        ...
        CareCloudCalendarKafkaResource kafkaResource = new CareCloudCalendarKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/care-cloud-calendar-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/care-cloud-calendar-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
